...
public class ApiRequest extends BaseApiRequest {
    ...
    public void setParameterIdRange(String key, Date beginDate, Date endDate) {
        if (key == null) {
            return;
        }
        ...
        if (beginDate != null) {
            begin = String.format("%s%013d", DateUtil.formatDate(beginDate, "yyMMdd"), 0);
        }
        if (endDate != null) {
            Calendar endDateCalendar = Calendar.getInstance();
            endDateCalendar.setTime(endDate);
            endDateCalendar.add(Calendar.DATE, 1);
            end = String.format("%s%013d", DateUtil.formatDate(endDateCalendar.getTime(), "yyMMdd"), 0);
        }

        if (begin != null || end != null) {
            this.setParameterBetween(key, begin, end);
        }
    }
	...
	public String toQueryString() {
		...
		try {
			sb.append("=").append(URLEncoder.encode(json.toString(), "UTF-8"));
